...
static double train_and_eval(Sequential& model,
    ...
    opt_cfg.lr = lr;
    opt_cfg.momentum = hpo::env_double("CNN_MOMENTUM", 0.9);
    opt_cfg.weight_decay = hpo::env_double("CNN_WEIGHT_DECAY", 0.0);
    opt_cfg.beta1 = hpo::env_double("CNN_BETA1", 0.9);
    opt_cfg.beta2 = hpo::env_double("CNN_BETA2", 0.999);
    opt_cfg.alpha = hpo::env_double("CNN_RMSPROP_ALPHA", 0.99);
    opt_cfg.eps = hpo::env_double("CNN_EPS", 1e-8);
    opt_cfg.grad_clip_norm = hpo::env_double("CNN_GRAD_CLIP", 0.0);
    ...
    benchlog::BenchBioHarness bio("cnn", variant, seed);
    bio.attach(model.parameters(), model.gradients(), "cnn_param");
    ...
    for (int epoch = 0; epoch < epochs; ++epoch) {
        ...
        for (size_t b = 0; b + batch_size <= train.images.rows; b += batch_size) {
            ...
            double loss = loss_fn->forward(probs, by);
            ...
            if (batch_idx % batch_log_every == 0 || batch_idx == total_batches) {
                logger.append_csv_row(
                    ...
                    {logger.run_id(), "cnn", variant, std::to_string(epoch + 1), std::to_string(batch_idx),
        ...
        for (size_t i = 0; i < params.size(); ++i) {
            const double pnorm = params[i]->norm();
            size_t nnz_small = 0;
            for (double v : params[i]->data) {
                if (std::abs(v) < 1e-3) nnz_small++;
            }
            const double sparsity =
                params[i]->data.empty() ? 0.0 : static_cast<double>(nnz_small) / params[i]->data.size();
            logger.append_csv_row(
                "model_specific/cnn/filter_stats.csv",
                {"run_id", "epoch", "layer", "l2"},
                {logger.run_id(), std::to_string(epoch + 1), "param_" + std::to_string(i), std::to_string(pnorm)}
            );
            logger.append_csv_row(
                "model_specific/cnn/featuremap_stats.csv",
                {"run_id", "epoch", "layer", "sparsity"},
                {logger.run_id(), std::to_string(epoch + 1), "param_" + std::to_string(i), std::to_string(sparsity)}
            );
            double energy = 0.0, abs_sum = 0.0;
            size_t tiny_count = 0;
            for (double v : params[i]->data) {
                energy += v * v;
                abs_sum += std::abs(v);
                if (std::abs(v) < 0.001) tiny_count++;
            }
            double mean_mag = params[i]->data.empty() ? 0.0 : abs_sum / params[i]->data.size();
            double frac_tiny = params[i]->data.empty() ? 0.0 : static_cast<double>(tiny_count) / params[i]->data.size();
            logger.append_csv_row(
                "model_specific/cnn/activation_energy.csv",
                {"run_id", "epoch", "layer", "activation_energy", "mean_magnitude", "fraction_tiny"},
                {logger.run_id(), std::to_string(epoch + 1), "param_" + std::to_string(i),
                 std::to_string(energy), std::to_string(mean_mag), std::to_string(frac_tiny)}
            );
        }
...
int main() {
    std::cout << "=== CNN Benchmark (MNIST) ===" << std::endl;
    const int seed = hpo::env_int("CNN_SEED", 42);
    std::mt19937 rng(static_cast<unsigned int>(seed));
    const int epochs = hpo::env_int("CNN_EPOCHS", 10);
    const size_t batch_size = hpo::env_size("CNN_BATCH_SIZE", 64);
    const double lr = hpo::env_double("CNN_LR", 0.001);
    const auto optimizer_type = hpo::parse_optimizer(hpo::env_string("CNN_OPTIMIZER", "adam"));
    const auto loss_type = hpo::parse_loss(hpo::env_string("CNN_LOSS", "cross_entropy"));
    const std::string variant_filter = hpo::to_lower(hpo::env_string("CNN_VARIANT", "all"));
    ...
    double min_acc = 1.0;
    bool ran_any = false;
    if (variant_filter == "all" || variant_filter == "simple" || variant_filter == "simple_conv") {
        std::mt19937 rng2(static_cast<unsigned int>(seed));
        auto model = build_simple_conv(rng2);
        ran_any = true;
        min_acc = std::min(
            min_acc,
            train_and_eval(
    ...
    if (variant_filter == "all" || variant_filter == "lenet" || variant_filter == "lenet5") {
        std::mt19937 rng2(static_cast<unsigned int>(seed));
        auto model = build_lenet5(rng2);
        ran_any = true;
        min_acc = std::min(
            min_acc,
            train_and_eval(
    ...
    if (variant_filter == "all" || variant_filter == "resnet" || variant_filter == "resnet_simple") {
        std::mt19937 rng2(static_cast<unsigned int>(seed));
        auto model = build_resnet_simple(rng2);
        ran_any = true;
        min_acc = std::min(
            min_acc,
            train_and_eval(
    ...
    if (!ran_any) {
        std::cerr << "No CNN variant selected for CNN_VARIANT='" << variant_filter
                  << "'. Valid values: all,simple,lenet,resnet" << std::endl;
        return 2;
    }
    std::cout << "\n=== CNN Benchmark Complete ===" << std::endl;
